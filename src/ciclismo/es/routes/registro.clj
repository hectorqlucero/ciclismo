(ns ciclismo.es.routes.registro
  (:require [cheshire.core :refer [generate-string]]
            [noir.response :refer [redirect]]
            [noir.session :as session]
            [selmer.parser :refer [render-file]]
            [clojure.java.io :as io]
            [ciclismo.models.crud :refer [db Query Save Update config]]
...
(defn buscar [request]
  (render-file "es/buscar.html" {:title "Busqueda de Registro"
...
(defn matricula [matricula]
  (let [row (first (Query db [matricula-sql matricula]))]
    (render-file "es/matricula.html" {:title "Registro De Alumno"
                                          :matricula matricula
...
(defn reset-jwt [token]
  (let [matricula (check-token token)]
    (if-not (nil? matricula)
      (render-file "es/reset_password.html" {:title "Resetear Contraseña"
